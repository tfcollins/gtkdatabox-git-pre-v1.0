...
static gint gtk_databox_ruler_draw (GtkWidget * widget, cairo_t * cr);
...
static void gtk_databox_ruler_get_preferred_height (GtkWidget * widget,
						    gint * minimal_height,
						    gint * natural_height);
static void gtk_databox_ruler_create_backing_surface (GtkDataboxRuler *
						      ruler);
static void gtk_databox_ruler_set_property (GObject * object, guint prop_id,
					    const GValue * value,
					    GParamSpec * pspec);
static void gtk_databox_ruler_get_property (GObject * object, guint prop_id,
					    GValue * value,
					    GParamSpec * pspec);
...
G_DEFINE_TYPE_WITH_PRIVATE (GtkDataboxRuler, gtk_databox_ruler,
			    GTK_TYPE_WIDGET)
     static void gtk_databox_ruler_class_init (GtkDataboxRulerClass * class)
{
   ...
   widget_class->get_preferred_height =
      gtk_databox_ruler_get_preferred_height;
   ...
   g_object_class_install_property (gobject_class,
				    ...
				    ("manual-tick-labels",
				     ...
				     G_PARAM_READWRITE));
   g_object_class_install_property (gobject_class, PROP_INVERT_EDGE,
				    g_param_spec_uint ("invert-edge",
						       "Invert Edge",
						       "Invert the Edge - the edge is drawn inverted: true or false",
						       FALSE, TRUE, FALSE,
						       G_PARAM_READWRITE));
   g_object_class_install_property (gobject_class, PROP_LINEAR_LABEL_FORMAT,
				    g_param_spec_string
				    ("linear-label-format",
				     "Linear Label Format",
				     ...
				     G_PARAM_READWRITE));
   g_object_class_install_property (gobject_class, PROP_LOG_LABEL_FORMAT,
				    g_param_spec_string ("log-label-format",
							 "Log Label Format",
							 "Log Label format mark up strings: marked up formatting strings for log labels (i.e. \"%%-%dg\")",
							 LOG_FORMAT_MARKUP,
							 G_PARAM_READWRITE));
   g_object_class_install_property (gobject_class, PROP_BOX_SHADOW,
...
gtk_databox_ruler_motion_notify (GtkWidget * widget, GdkEventMotion * event)
{
   ...
   if (gtk_databox_ruler_get_draw_position (ruler))
   {
	 ...
	 ruler->priv->position =
	    ruler->priv->lower +
	    ((ruler->priv->upper -
	      ruler->priv->lower) * x) / allocation.width;
      else
	 ruler->priv->position =
	    ruler->priv->lower +
	    ((ruler->priv->upper -
	      ruler->priv->lower) * y) / allocation.height;
				...
				const GValue * value, GParamSpec * pspec)
{
   ...
   case PROP_DRAW_POSITION:
      gtk_databox_ruler_set_draw_position (ruler,
					   (gboolean)
   ...
   case PROP_TEXT_ORIENTATION:
      gtk_databox_ruler_set_text_orientation (ruler,
					      (GtkOrientation)
					      g_value_get_uint (value));
      break;
   case PROP_TEXT_ALIGNMENT:
      gtk_databox_ruler_set_text_alignment (ruler,
					    (PangoAlignment)
					    g_value_get_uint (value));
      break;
   case PROP_TEXT_HOFFSET:
      gtk_databox_ruler_set_text_hoffset (ruler,
					  (GtkOrientation)
					  g_value_get_uint (value));
      break;
   case PROP_DRAW_TICKS:
      gtk_databox_ruler_set_draw_ticks (ruler,
					(gboolean)
					g_value_get_boolean (value));
      break;
   case PROP_DRAW_SUBTICKS:
      gtk_databox_ruler_set_draw_subticks (ruler,
					   (gboolean)
					   g_value_get_boolean (value));
      break;
   case PROP_MANUAL_TICKS:
      gtk_databox_ruler_set_manual_ticks (ruler,
					  (gfloat *)
   ...
   case PROP_MANUAL_TICK_LABELS:
      gtk_databox_ruler_set_manual_tick_labels (ruler,
						(gchar **)
						g_value_get_pointer (value));
      break;
   case PROP_INVERT_EDGE:
      gtk_databox_ruler_set_invert_edge (ruler,
					 (gboolean)
					 g_value_get_boolean (value));
      break;
   case PROP_LINEAR_LABEL_FORMAT:
      gtk_databox_ruler_set_linear_label_format (ruler,
						 (gchar *)
						 g_value_get_string (value));
      break;
   case PROP_LOG_LABEL_FORMAT:
      gtk_databox_ruler_set_log_label_format (ruler,
					      (gchar *)
					      g_value_get_string (value));
      break;
   case PROP_BOX_SHADOW:
      gtk_databox_ruler_set_box_shadow (ruler,
					(GtkShadowType)
					g_value_get_uint (value));
      break;
   default:
				   ...
				   GtkOrientation orientation)
{
   ...
   else
   {
	 ...
	 gtkRequisition.width =
	    (padding.left + padding.right) * 2 + RULER_SIZE;
      else
	 gtkRequisition.width =
	    (padding.left + padding.right) + ruler->priv->max_y_text_width;
   }
   gtk_widget_set_size_request (widget, gtkRequisition.width,
...
static void
gtk_databox_ruler_get_preferred_width (GtkWidget * widget,
				       gint * minimal_width,
				       gint * natural_width)
{
...
static void
gtk_databox_ruler_get_preferred_height (GtkWidget * widget,
					gint * minimal_height,
					gint * natural_height)
{
   GtkDataboxRuler *ruler = GTK_DATABOX_RULER (widget);
					...
					GtkOrientation orientation)
{
   ...
   gtk_style_context_get_padding (context,
				  gtk_widget_get_state_flags (widget),
				  &padding);
   gtk_databox_ruler_get_preferred_height (widget, &minimal_height,
					   &natural_height);
 ...
 **/
void
gtk_databox_ruler_set_text_hoffset (GtkDataboxRuler * ruler, gint offset)
{
   g_return_if_fail (GTK_DATABOX_IS_RULER (ruler));
 ...
 **/
void
gtk_databox_ruler_set_manual_tick_labels (GtkDataboxRuler * ruler,
					  gchar ** manual_tick_labels)
{
   g_return_if_fail (GTK_DATABOX_IS_RULER (ruler));
 ...
 **/
void
gtk_databox_ruler_set_linear_label_format (GtkDataboxRuler * ruler,
					   gchar * format)
{
   g_return_if_fail (GTK_DATABOX_IS_RULER (ruler));
   ...
   if (g_strcmp0 (ruler->priv->linear_format, format) != 0)
   {
      ...
      {
	 g_warning
	    ("maximum format length = %d chars exceeded, truncating to the maximum from %d",
 ...
 **/
void
gtk_databox_ruler_set_log_label_format (GtkDataboxRuler * ruler,
					gchar * format)
{
   g_return_if_fail (GTK_DATABOX_IS_RULER (ruler));
...
gtk_databox_ruler_draw_ticks (GtkDataboxRuler * ruler)
{
   ...
   GtkBorder padding;
   gtk_style_context_get_padding (stylecontext,
				  gtk_widget_get_state_flags (widget),
				  &padding);
   xthickness = padding.left + padding.right;
   ythickness = padding.top + padding.bottom;
   ...
   if (ruler->priv->scale_type == GTK_DATABOX_SCALE_LINEAR)
	 ...
	 g_snprintf (format_string, FORMAT_LENGTH, ruler->priv->linear_format,
		     ruler->priv->max_length);
      else
	 g_snprintf (format_string, FORMAT_LENGTH, ruler->priv->linear_format,
		     ruler->priv->max_length - 1);
   else if (ruler->priv->max_length == 1)
      g_snprintf (format_string, FORMAT_LENGTH, ruler->priv->log_format,
		  ruler->priv->max_length);
   else
      g_snprintf (format_string, FORMAT_LENGTH, ruler->priv->log_format,
		  ruler->priv->max_length - 1);
   ...
   {				/* we are manually setting the tick labels and marks. */
      start = 0.;
   ...
   for (cur = start; cur <= end; cur += subd_incr)
   {
	 ...
	 cur_text = ruler->priv->manual_ticks[(int) cur];
      ...
      {
	 if (ruler->priv->orientation == GTK_ORIENTATION_HORIZONTAL)
	    cairo_rectangle (cr, pos, height + ythickness - length, 1,
			     length);
	 else
	 ...
	 pango_layout_set_text (layout,
				ruler->priv->manual_tick_labels[(int) cur],
				-1);
      else
      {
	 if ((ruler->priv->scale_type == GTK_DATABOX_SCALE_LINEAR)
	 ...
	 pango_layout_set_text (layout, unit_str, -1);
      ...
      if ((ruler->priv->orientation ==
	   GTK_ORIENTATION_VERTICAL) & (ruler->priv->max_y_text_width <
					logical_rect.width))
      {
	 ...
	 gtk_widget_set_size_request (GTK_WIDGET (ruler),
	 ...
	 gtk_render_layout (stylecontext, cr, pos + 2, ythickness - 1,
			    layout);
      }
      else
      {
	 ...
	 if ((ruler->priv->text_orientation == GTK_ORIENTATION_HORIZONTAL) & (ruler->priv->text_alignment == PANGO_ALIGN_CENTER))	/* set centrally adjusted text */
	    x_loc =
	       (width - ink_rect.width) / 2 - 2 + ruler->priv->text_hoffset;
	 ...
	 if (ruler->priv->scale_type == GTK_DATABOX_SCALE_LINEAR)
	    for (i = 1; i < 5; ++i)
	    {
		  ...
		  cairo_rectangle (cr, pos,
				   height + ythickness - subtick_start, 1,
				   length / 2);
	       else
		  cairo_rectangle (cr, width + xthickness - subtick_start,
				   pos, length / 2, 1);
	    }
	 else if (ruler->priv->scale_type == GTK_DATABOX_SCALE_LOG2)
	    for (i = 1; i < 8; ++i)
...
gtk_databox_ruler_draw_pos (GtkDataboxRuler * ruler)
{
   ...
   if (gtk_widget_is_drawable (widget))
   {
	 ...
	 return;		/* return if positive values and inverted */
      ...
      drawc =
	 gdk_window_begin_draw_frame (gtk_widget_get_window (widget), crr);
...
/* TODO: HERE WE SHOULD REALLY CLIP THIS TO THE RECTANGLE OF THE ARROW */
	 ...
	 cairo_paint (cr);
      ...
      {
	 increment =
	    (gdouble) width / (ruler->priv->upper - ruler->priv->lower);

	 x = ROUND ((ruler->priv->position -
		     ruler->priv->lower) * increment) + (xthickness -
							 bs_width) / 2 - 1;
	 ...
	 cairo_line_to (cr, x + bs_width, y);
      ...
      {
	 increment =
	    (gdouble) height / (ruler->priv->upper - ruler->priv->lower);

	 x = (width + bs_width) / 2 + xthickness;
	 y = ROUND ((ruler->priv->position -
		     ruler->priv->lower) * increment) + (ythickness -
							 bs_height) / 2 - 1;
	 ...
	 cairo_line_to (cr, x, y + bs_height);
      ...
      ruler->priv->ysrc = y;
      if (ruler->priv->invert_edge)
      {				/* inverted edges need clearing in the negative direction */
	 if (ruler->priv->orientation == GTK_ORIENTATION_HORIZONTAL)
